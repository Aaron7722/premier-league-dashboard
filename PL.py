...
selected_teams = st.sidebar.multiselect(
    "Highlight specific teams (optional)",
    options=league_table['Team'].tolist(),
    default=[]
)
...
with tab1:
    ...
    colors = ['#d62728' if t in selected_teams else None for t in league_table['Team']]
    sns.barplot(data=league_table, x='Points', y='Team', hue='Team',
...
with tab2:
    ...
    sns.scatterplot(data=league_table, x='GF', y='GA', s=100, ax=ax)
...
with tab4:
    fig, ax = plt.subplots(figsize=(10, 8))
    top_scoring = league_table.sort_values('GF', ascending=False)
